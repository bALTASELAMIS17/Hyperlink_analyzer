...
// returns 1 if url contains certain words
int url_has_banned_word(const char *url) {
    ...
    int n = sizeof(banned_words) / sizeof(banned_words[0]);

    for (int i = 0; i < n; i++) {
        if (strstr(url, banned_words[i]) != NULL) {
            return 1;
        }
    }
    return 0;
...
int extract_hyperlinks(const char *filepath, char ***labels_out, char ***links_out) {
    ...
    while (fgets(file_line, sizeof(file_line), fp) != NULL) {
		...
		while ((line_pointer = strstr(line_pointer, "<a href=\"/wiki")) != NULL) {
			...
			if (url_has_banned_word(link)) {
			    free(link);
			    line_pointer = link_end + 1;
			    continue;
			}
